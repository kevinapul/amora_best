<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($event->jenis_event === 'inhouse')
                ...
                @can('approveFinance', $event)
                    @if ($event->status === 'done' && !$event->finance_approved)
                        <form method="POST" action="{{ route('event-training.approveFinance', $event->id) }}"
                            class="mb-4">
                            @csrf
                                    ...
                                    @if ($event->jenis_event === 'reguler')
                                        ...
                                        <td class="border px-3 py-2 text-center">
                                            @if ($participant->pivot->is_paid)
                                                <span class="px-2 py-1 rounded bg-green-600 text-white font-semibold">✔
                                                    Lunas</span>
                                            @else
                                                <span
                                                    class="px-2 py-1 rounded bg-red-600 text-white font-semibold">Belum</span>
                                            @endif
                                        </td>
                                    @endif
                                </tr>
            ...
            @if ($event->jenis_event === 'reguler' && $event->status === 'done' && !$event->finance_approved)
