<x-app-layout>
        ...
        <div class="max-w-6xl mx-auto space-y-8">
            ...
            <form action="{{ route('event-participant.store', $event) }}" method="POST"
                ...
                <div class="bg-white border rounded-2xl p-6 shadow-sm">
                    ...
                    <input type="text" name="perusahaan" class="alkon-input w-full"
                        ...
                        <div class="participant-card p-5 rounded-xl">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input name="participants[0][nama]" class="alkon-input" required>
                                </div>

                                <div>
                                    <label class="alkon-label">No HP</label>
                                    <input name="participants[0][no_hp]" class="alkon-input">
                                </div>

                                <div>
                                    <label class="alkon-label">NIK</label>
                                    <input name="participants[0][nik]" class="alkon-input nik-input"
                                        onkeyup="checkNik(this)">
                                    <p class="text-red-500 text-xs mt-1 hidden nik-warning">
                                        NIK sudah terdaftar
                                    </p>
                                </div>
                                <div class="text-xs text-gray-500 mt-1 history-box hidden"></div>


                                <div>
                                    <label class="alkon-label">Jenis Layanan *</label>
                                    <select name="participants[0][jenis_layanan]" class="alkon-input" required>
                                ...
                                @if ($event->isReguler())
                                    <div class="col-span-2">
                                        ...
                                        <input type="number" name="participants[0][harga_peserta]"
                                            class="alkon-input text-right harga-input" oninput="updateTotal()"
                                            min="0" required>
                                    </div>
                                @endif
                            </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            <label class="alkon-label">NIK</label>
            <input name="participants[${index}][nik]" class="alkon-input nik-input" onkeyup="checkNik(this)">
            <p class="text-red-500 text-xs mt-1 hidden nik-warning">
                NIK sudah terdaftar
            </p>
        </div>
        ...
        <div>
            <label class="alkon-label">Jenis Layanan *</label>
            <select name="participants[${index}][jenis_layanan]" class="alkon-input" required>
                <option value="">-- Pilih --</option>
                <option value="pelatihan">Pelatihan</option>
    ...
    <button type="button"
            onclick="this.parentElement.remove(); checkAllNik(); updateTotal();"
    ...
    `;
        ...
        function checkNik(el) {
            const nik = el.value.trim();
            const warning = el.parentElement.querySelector('.nik-warning');
            ...
            nikTimer = setTimeout(() => {
                ...
                fetch(`/check-nik?nik=${nik}&event_id={{ $event->id }}`)
                    .then(r => r.json())
                    .then(res => {

                        if (res.already_in_event) {
                            el.classList.add('border-red-500');
                            warning.innerText = "Peserta sudah terdaftar di training ini";
                            ...
                            if (res.exists) {
                                ...
                                const hpInput = card.querySelector('input[name*="[no_hp]"]');
                                const perusahaanInput = document.querySelector('input[name="perusahaan"]');

                                if (namaInput && !namaInput.value)
                                    namaInput.value = res.nama ?? '';

                                if (hpInput && !hpInput.value)
                                    hpInput.value = res.no_hp ?? '';

                                if (perusahaanInput && !perusahaanInput.value && res.company)
                                    perusahaanInput.value = res.company;

                                // tampilkan history
                                const historyBox = card.querySelector('.history-box');
                                if (historyBox && res.history.length) {
                                    let html = "<b>Riwayat training:</b><br>";
                                    res.history.forEach(h => {
                                        html += `• ${h.name} (${h.tanggal_start})<br>`;
                                    });
        ...
        function checkAllNik() {
            const errors = document.querySelectorAll('.nik-input.border-red-500');
